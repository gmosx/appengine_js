 ...
 * group are ACID.
 */
var Model = exports.Model = function(properties) {
...
var initializeProperties = exports.initializeProperties = function(modelClass, properties) {
...
Model.extend = function(data) {
    var ctor = function(data) {
        if (data) {
            if (data.key) {
                ...
                this._key = Key.create({kind: this.constructor.kind(), parent: data.parent, name: data.keyName});
                delete data.keyName;
                delete data.parent;
            }
    ...
    if (data.kind) {
        ctor._kind = data.kind;
    	db.kindMap[ctor._kind] = ctor;        
        delete data.kind;    
    } else {
        throw new TypeError("Kind not defined for Model");
    }
    
    ctor._properties = Hash.merge(this.properties(), data);

    initializeProperties(ctor, ctor._properties);
...
Model.getByKeyName = function(names, parent) {
	...
	if (Array.isArray(names)) {
        keys = names.map(function(n) { new Key({parent: parent, kind: this.kind(), name: n}) });
	} else {
...
Model.prototype._populateEntity = function() {
    ...
    if (this.isSaved()) {
        ...
        var key = this._key || Key.create({kind: this.constructor.kind()});
...
Model.prototype.key = function() {
    if (this.isSaved()) {
        return Key.fromDatastoreKey(this._entity.getKey());
    } else if (this._key) {
        return this._key;
    } else {
