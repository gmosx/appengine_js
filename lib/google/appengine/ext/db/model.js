...
/**
 * Lookup up an instance by the string encoded key.
 * Python API extension.
 */
Model.getByEncodedKey = function(encoded) {
    if (isArray(encoded)) {
        return this.get(encoded.map(function(k) { new db.Key(k) }));
    } else {
        return this.get(new db.Key(encoded));
    }
}
