...
var entityToObject = function(entity) {
	...
	var obj = new constructor(); // FIXME: allocate don't call new! (Object.create)
...
exports.model = function(constructor, kind, properties, model) {
	...
	constructor.prototype.setKey = function(key) {
		this.__key__ = key;
	}
	...
	if (undefined == constructor.createEntity)
		model.createEntity = function(obj) { 
			return new Entity(kind, obj.__key__.getName(), obj.__key__.getParent()); 
		}
/*		
		var np = model.nameProperty, pp = model.parentProperty;
		
		if (np & pp) 
			model.createEntity = function(obj) { return new Entity(kind, obj[np], obj[pp]) }
		else if (np)
			model.createEntity = function(obj) { return new Entity(kind, obj[np]) }
		else if (pp)
			model.createEntity = function(obj) { return new Entity(kind, obj[pp]) }
		else
			model.createEntity = function(obj) { return new Entity(kind) }
	}
*/	
