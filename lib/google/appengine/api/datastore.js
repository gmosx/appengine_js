...
var JDATASTORE = Packages.com.google.appengine.api.datastore,
    ...
    JAsyncDatastore = exports.AsyncDatastore = JDatastoreServiceFactory.getAsyncDatastoreService();
...
DatastoreRPC.prototype.getDatastore = function () {
    if (this.args) {
        ...
        // Returns a DatastoreService using the provided config.
        return JDatastoreServiceFactory.getDatastoreService(config);
    } else {
        // Returns a DatastoreService using the default config (DatastoreServiceConfig.Builder.withDefaults()).
        return JDatastoreServiceFactory.getDatastoreService();
    }   
}

DatastoreRPC.prototype.getAsyncDatastore = function () {
    if (this.args) {
        var config = JDatastoreServiceConfigBuilder.withReadPolicy(this.args.readPolicy || exports.STRONG_CONSISTENCY);
        if (this.args.deadline) {
            config.deadline(this.args.deadline);
        }
        //  Returns a AsyncDatastoreService using the provided config.
        return JDatastoreServiceFactory.getAsyncDatastoreService(config);
    } else {
        //  Returns a AsyncDatastoreService using the default config (DatastoreServiceConfig.Builder.withDefaults()).
        return JDatastoreServiceFactory.getAsyncDatastoreService();
    }   
}
